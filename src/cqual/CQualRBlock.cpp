 ...
 */
CQualRBlock::CQualRBlock(int lossyparam, int m, cds_word sample){
	mode = m;
	lossy = lossyparam;
	tmp_file_string = "";
	buffer = NULL;
...
CQualRBlock::~CQualRBlock(){
	if(IndexLine != NULL)
		delete [] IndexLine;
}

void CQualRBlock::ProcessLine(string qual){
	cds_word localmin = 256, localmax = 0, occ = 0;
	cds_word min = 0, max = 0, valueL = 0, storequal = 0;
	double r = 1.0 + lossy * 1.0 / 100.0;
	...
	if(variableSize == 0){
		if(sizeQualLine == 0){ //first line to be read
			...
			tmp_name = "tmp_rqual_" + to_string(pi); //should work
	...
	for(cds_word w = 0; w < qual.length(); w++){
		...
		if(min == 0){
			...
			if(valueL >= min && valueL <= max)
				...
				if(valueL < min  &&  isPossible(valueL, max, r)){
					occ++;
					min = valueL;
				}
				else if(valueL > max  &&   isPossible(min, valueL, r)){
					occ++;
					max = valueL;
				}
				else{
					numberOfValues++;
					storequal = computeMidPoint(min, max, r);
	...
	//check tmp_file_string and index
	check_tmp_string(&tmp_file_string, fileQual);
	numberOfLines ++;
}

void CQualRBlock::CreateFile(ofstream &fp){
	mem_run = mem_rep = 0;
	size_t IndexPos = 0;
	...
	buffer = new cds_word[ (MAXRAM + kWordSize * 200)  / kWordSize]; 
	buffer_use = 0;
	for(cds_word w = 0; w < ((MAXRAM + kWordSize * 200)  / kWordSize); w++)
		buffer[w] = 0;	
...
CQualRBlock * CQualRBlock::Load(ifstream &fp){
	...
	CQualRBlock *cqual = new CQualRBlock(0, 0, 0);
...
bool CQualRBlock::isPossible(cds_word min, cds_word max, double r){
	cds_word midpoint, lastpoint, m = min - 33, M = max - 33;
	cds_word lower = (cds_word)(ceil(M / r));
	midpoint = (cds_word)(m * r);
	lastpoint = (cds_word)(midpoint * r);
	if( M <= lastpoint ){
		if(lower < m)
			lower = m;
		if(midpoint > M)
...
cds_word CQualRBlock::computeMidPoint(cds_word min, cds_word max, double r){
	cds_word m = min - 33, M = max - 33;
	cds_word lower = (cds_word)(ceil(M / r));
	cds_word higher = (cds_word)(m * r);
	if(lower < m)
		lower = m;
	if(higher > M)
		higher = M;
	return (cds_word)(sqrt(lower * higher) + 33);
}
